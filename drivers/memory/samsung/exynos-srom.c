				      ...
				      struct device_node *np)
{
    ...
    if (of_property_read_bool(np, "samsung,waitable"))
		cs |= 1 << EXYNOS_SROM_BW__WAITENABLE__SHIFT;
	if (width == 2)
	{
        cs |= 1 << EXYNOS_SROM_BW__DATAWIDTH__SHIFT;
        cs |= 1 << EXYNOS_SROM_BW__ADDRMODE__SHIFT;
	}
//    dev_info(srom->dev, "cs =%#x bank=%#x\n", cs, bank);
//    dev_info(srom->dev, "timing =%#x %#x %#x %#x %#x %#x\n", timing[0],timing[1],timing[2],timing[3],timing[4],timing[5]);
